...
import org.apache.hadoop.io.{WritableComparable, Writable, IntWritable}
...
object WikipediaTitles extends ScoobiApp {
  def run() {
    ...
    // val fs = this.configuration.fileSystem
    // val reader = new SequenceFileReader(config.fileSystem,
      // new Path("enwiki-latest-docno.dat"), config)

    // val keyClass = (WritableComparable) reader.getKeyClass().newInstance()
    // IntWritable
    // val valueClass = (Writable) reader.getValueClass().newInstance()
    // WikipediaPage
    ...
    val pages = input.filter { case (intwritable, page) =>
      // println()
      // valpage.getDocid()
      // intwritable + "\t" + valpage.getDocid() + "\t" + valpage.getTitle()
    ...
    //   println(key)
    //   println(value.getDocid() + ": " + value.getTitle())
    //   // do something
    // }
    // reader.close();
